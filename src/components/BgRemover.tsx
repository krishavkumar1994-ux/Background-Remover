...
export default function BgRemover() {
  ...
  const processImage = async (imageSource: File) => {
    ...
    setProgressMsg('Initializing WebAssembly AI Engine...');
    ...
    try {
      ...
      const config = {
        progress: (key: string, current: number, total: number) => {
          ...
          if (key.includes('fetch')) {
            setProgressMsg(`Downloading AI model weights (${pct}%) - only happens on first run...`);
          } else if (key.includes('processing')) {
            setProgressMsg(`Analyzing image pixels and edges (${pct}%)...`);
          } else {
            setProgressMsg(`Processing background removal: ${key.split(':').pop() || key} (${pct}%)`);
          }
        }
      };
      ...
      setError(
        err?.message || 
        'Could not initialize the background removal model. This can happen on older browsers or if WebAssembly is disabled.'
      );
      setLoading(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-slate-500 font-medium text-lg">
            Experience studio-quality background removal running 100% in your browser. Private, fast, and high-resolution.
          </p>
        </div>
